-- bp_inventory (0RESMON "0DAY BP INVENTORY") bridge — SERVER.
--
-- bp_inventory is a grid inventory that ships an ox_inventory-compatible layer
-- and declares `provide 'ox_inventory'`, so `exports.ox_inventory:*` resolves to
-- bp's implementation. Confirmed in the wild by jayxjay2284 (qbx_core + bp):
-- "script works fine just the images" running through our ox bridge. So this
-- bridge is a straight clone of the ox_inventory bridge — we deliberately call
-- `exports.ox_inventory`, NOT `exports.bp_inventory`: bp's OWN documented exports
-- are a QB-flavored subset (AddItem/RemoveItem/GetItemByName/hasItem/getWeight)
-- that lacks CanCarryItem/GetItem/GetSlot/SetMetadata/Items — those only exist on
-- the ox-compat layer, under the ox_inventory export name.
--
-- The ONLY reason bp gets its own bridge (instead of being detected as
-- ox_inventory) is the item image path: bp serves icons from its own resource,
-- not nui://ox_inventory/. autodetect.lua maps bp_inventory → its own imagePath,
-- which is what fixes the blank fish-store images.
